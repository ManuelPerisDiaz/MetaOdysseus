plot_detection<-function (file,norm=FALSE, do.plot = TRUE, t1 = 0, t2 = 0,csv=FALSE,orig=TRUE,QC1=TRUE,QC2=TRUE,dir="C:")
{
  ...
  if (do.plot == TRUE ) {
    ...
    d22 <- d22 + xlab("m/z")
    on.exit(par(opar))
    ggsave(d22,filename=file.path(dir,"MS_detection.tiff"), dpi = 800, width = 6, height = 4, units = 'in')
    ...
    if(QC1==TRUE){
      ...
      d22 <- d22 + xlab("m/z")
      on.exit(par(opar))
      ggsave(d22,filename=file.path(dir,"QC1.tiff"), dpi = 800, width = 6, height = 4, units = 'in')
    ...
    if(QC2==TRUE){
      ...
      ggsave(d22,filename=file.path(dir,"QC2.tiff"), dpi = 800, width = 6, height = 4, units = 'in')
