targetMSMS_permu<-function (Deconv_CysMpro, permutation,theory, txt=TRUE,t = 1, b = 5,plot=FALSE, supress = FALSE,xlim = c(100,2500),label = "")
{

  theory<- data.frame(theory)

  if(permutation==TRUE){
    expt<-data.frame(mz=as.numeric(Deconv_CysMpro[1]),intensity=as.numeric(Deconv_CysMpro[2]))
    expt$normalized <- as.numeric(Deconv_CysMpro[2])

    splited<-strsplit(as.character(Deconv_CysMpro$ms2type)[1],
                      split = "")[[1]]

    charge<-splited[6]

    expt<-cbind(expt,carga=as.numeric(charge),Totalscore=Deconv_CysMpro$score)
  ...
  for (i in 1:nrow(expt)) {
    ...
    tmp_matchesb <- theory[expt$mz[i] >= theory$ms2mz - t &
  ...
  row_sub = apply(  identifications, 1, function(row) all(row !=0 ))
  identifications <- identifications[row_sub,]
